...
import { hexReverse } from './utils-merkle';
...
export function getRoot(txId:string, block:any):string {
  // coinmonks...
  const txs = block.tx as string[];
  const txIndex = txs.findIndex((t:any) => t.txid === txId);
  const reversedTxIds = txs.map(function(tx:any) {
    return hexReverse(tx.hash)
  });
  const leaves = reversedTxIds.map(x => sha256(sha256(x)))
  const tree = new MerkleTree(leaves, sha256)
  const root = tree.getRoot().toString('hex')
  const leaf = sha256(sha256(reversedTxIds[txIndex]))
  const proof = tree.getProof(hex.encode(leaf))
  console.log(tree.verify(proof, hex.encode(leaf), root)) // true
  return root
}
