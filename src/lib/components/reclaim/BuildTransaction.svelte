...
import PegInAmount from "./TransactionId.svelte";
import AddressTransactions from "./AddressTransactions.svelte";
import { createEventDispatcher } from "svelte";
import PegInTransaction from '$lib/domain/PegInTransaction';
import type { PegInTransactionI } from '$lib/domain/PegInTransaction';
import { addresses } from '$lib/stacks_connect'
import { truncate, explorerBtcAddressUrl, explorerBtcTxUrl } from "$lib/utils";
import { fetchAddressTransactions } from '$lib/bridge_api.js'
...
const addressTransactions = async () => {
  errorReason = undefined;
  if (!piTx.fromBtcAddress) return;
  try {
    txs = await fetchAddressTransactions(piTx.fromBtcAddress);
  } catch(err:any) {
...
{#if inited}
  <div class="mb-4"><AddressTransactions {utxoData} on:utxo_updated={utxoUpdated} /></div>
  ...
  <div class="mt-3 mb-4 mx-0 px-0">
    ...
    <div class="row text-white text-small">
      <div class="col-2"><span class="">{tx.vout}</span></div>
      <div class="col-2"><span class="">{tx.locktime}</span></div>
      <div class="col-2"><span class={(tx.confirmations < 6) ? 'text-warning' : 'text-success'}>{tx.confirmations} {(tx.confirmations < 6) ? '(of 6)' : ''}</span></div>
      <div class="col-6"><span class=""><a href={explorerBtcTxUrl(tx.txid)} target="_blank" rel="noreferrer">{truncate(tx.txid, 10)}</a></span></div>
    </div>
    {/each}
  </div>
