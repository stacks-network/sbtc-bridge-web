...
import { explorerBtcTxUrl, parseOutputs } from '$lib/utils'
...
onMount(() => {
	let script;
	try {
    script = hex.decode(peginRequest.commitTxScript!.witnessScript!);
    const scriptDec = btc.Script.decode(script);
    const d1U = scriptDec[0].valueOf() as Uint8Array;
...
<div class="row text-small">
  <div class="col-12 mt-0 mb-2">Commit Data</div>
  <div class="col-md-2 col-sm-12 text-info">Return Address</div><div class="col-md-10 col-sm-12">{peginRequest.fromBtcAddress}</div>
