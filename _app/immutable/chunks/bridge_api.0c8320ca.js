...
async function fetchUtxoSet(address) {
  const path = "http://34.135.65.158/bridge-api/v1" + "/btc/wallet/address/" + address + "/utxos?verbose=true";
  const response = await fetch(path);
  if (response.status !== 200) {
    throw new Error("Bitcoin address not known - is the network correct?");
  }
...
export { fetchSbtcEvents as a, fetchUtxoSet as b, fetchCurrentFeeRates as c, fetchUserSbtcBalance as d, fetchSbtcData as f, sendRawTxDirectMempool as s };
